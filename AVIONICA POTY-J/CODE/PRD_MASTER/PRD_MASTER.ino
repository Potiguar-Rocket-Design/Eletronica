...
#define disk1 0x50                        // endereço da EEPROM
#define ledSD 7
#define ledSENSOR 3
#define ledRTC 5
#define buzzer 3
...
const int chipSelect = 10;            // select do sd card
...
void setup()
{
  ...
  Serial.print("INICIALIZANDO O CARD..");

  // see if the card is present and can be initialized:
  if (!SD.begin(chipSelect)) {
    Serial.println("FALHA  NO CARTAO, OU NAO ESTA INSERIDO");
...
void loop()
{
  ...
  // Comunicação Serial i2c com outro arduino
  ...
  /*    
     if(start - guardar >= 100)// passivo de ajuste
    {
     writeEEPROM(disk1, address, P);// grava na memoria eeprom externa dados de altitude
     address += sizeof(float); // 4/Move address to the next byte after float 'f'.
     guardar = start;
    }
